...
- `delete/trash/move/overwrite-risk`、database/data source の schema・構造変更、`Status=approved` を含む publish 相当は毎回ユーザー明示確認が必要。ただし `scripts/apply_post_package.py --vote-input ... --consensus-approve` が有効な `x-auto.consensus.receipt/v1` を検証し、runtime publish gate を通過した場合は合議承認として扱える。
- ユーザーが `approve` / `publish` / `schedule-as-approved` を明示せず、有効な consensus approval receipt も無い場合、既定は `Status=draft`。

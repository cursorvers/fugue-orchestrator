...
failed=0
total=0
CLAUDE_EXPECTED_DEFAULT="claude-opus-4-6"
...
assert_model "default-claude" \
  "claude_model" "${CLAUDE_EXPECTED_DEFAULT}"

assert_model "default-claude-cli" \
  "claude_cli_model" "${CLAUDE_EXPECTED_DEFAULT}"

assert_model "default-claude-api" \
  "claude_api_model" "${CLAUDE_EXPECTED_DEFAULT}"
...
assert_model "codex-multi-codex-valid" \
  "codex_multi_agent_model" "gpt-5-codex" \
  --codex-multi-agent-model "gpt-5-codex"
...
assert_model "claude-sonnet-valid" \
  "claude_model" "${CLAUDE_EXPECTED_DEFAULT}" \
  --claude-model "${CLAUDE_EXPECTED_DEFAULT}"

assert_model "claude-api-valid" \
  "claude_api_model" "${CLAUDE_EXPECTED_DEFAULT}" \
  --claude-model "${CLAUDE_EXPECTED_DEFAULT}"

assert_model "glm-5-valid" \
  "glm_model" "glm-5" \
  --glm-model "glm-5"

assert_model "glm-5.0-normalizes" \
  "glm_model" "glm-5" \
  --glm-model "glm-5.0"
...
assert_model "adjusted-true" \
  ...
  --codex-main-model "gpt-4" --claude-model "${CLAUDE_EXPECTED_DEFAULT}"

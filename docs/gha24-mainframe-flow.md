...
- **Claude throttle guard.** `FUGUE_CLAUDE_RATE_LIMIT_STATE={ok|degraded|exhausted}` provides an operations kill-switch; `exhausted` routes main `claude -> codex`, assist `claude -> none` unless per-issue forced. `ok/degraded` keeps Claude assist lanes available.

...
- **Router skip.** `fugue-task-router` short-circuits whenever an issue already carries `tutti` or `processing`, and it also skips issues using the `## GHA24 Task` header (those are expected to be driven by `scripts/gha24`). This keeps the "plain fugue-task router" lane and the "GHA24 mainframe" lane from competing. `fugue-tutti-router` also skips reprocessing while `processing` is attached, except when `FUGUE_EMERGENCY_CONTINUITY_MODE=true` (inflight-only continuation).

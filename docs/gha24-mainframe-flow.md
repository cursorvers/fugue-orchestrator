...
2. **Tutti consensus.** Adding label `tutti` fires `.github/workflows/fugue-tutti-caller.yml`, which resolves main/assist orchestrators with this precedence: issue labels → body hints → repo defaults (`FUGUE_MAIN_ORCHESTRATOR_PROVIDER`, `FUGUE_ASSIST_ORCHESTRATOR_PROVIDER`, legacy fallback `FUGUE_ORCHESTRATOR_PROVIDER`). If `FUGUE_CLAUDE_RATE_LIMIT_STATE` is `degraded`/`exhausted` and not forced, main `claude` falls back to `codex`. For assist `claude`, fallback to `none` is applied only when state is `exhausted`. Then `.github/workflows/fugue-tutti-router.yml` runs a **6-lane baseline vote** (Codex3 + GLM3) and adds optional specialist lanes (Claude Opus/Sonnet assist, Gemini, xAI) as needed. Lane execution uses `FUGUE_CI_EXECUTION_ENGINE` (`harness` default, `api` fallback), and lane depth is controlled by `FUGUE_MULTI_AGENT_MODE` (`standard|enhanced|max`, default `enhanced`).

...
- **Claude throttle guard.** `FUGUE_CLAUDE_RATE_LIMIT_STATE={ok|degraded|exhausted}` provides an operations kill-switch. With `FUGUE_CLAUDE_ROLE_POLICY=flex` (default), codex/claude main switching is allowed; use `sub-only` to enforce `main claude -> codex` unless forced. Assist `claude` degrades via `FUGUE_CLAUDE_DEGRADED_ASSIST_POLICY={none|codex}` on `degraded` and is forced to `none` on `exhausted` unless per-issue forced.

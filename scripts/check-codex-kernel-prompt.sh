...
KERNEL_RUN_COMPLETE_FILE="${ROOT_DIR}/scripts/lib/kernel-run-complete.sh"
CODEX_VALIDATE_FILE="${ROOT_DIR}/scripts/harness/codex-execute-validate.sh"
...
assert_file "${KERNEL_RUN_COMPLETE_FILE}"
assert_file "${CODEX_VALIDATE_FILE}"
...
assert_contains "${AGENTS_FILE}" "Completion reporting:" "AGENTS documents completion reporting section"
assert_contains "${AGENTS_FILE}" "must always display a completion report" "AGENTS requires completion report"
assert_contains "${AGENTS_FILE}" "dirty worktree entries" "AGENTS requires dirty worktree disclosure"
assert_contains "${CODEX_VALIDATE_FILE}" "git status --porcelain" "Codex implement validator checks dirty worktree"
assert_contains "${CODEX_VALIDATE_FILE}" "no_changes=true" "Codex implement validator reports clean worktree"
assert_contains "${CODEX_VALIDATE_FILE}" "no_changes=false" "Codex implement validator reports dirty worktree"
assert_contains "${KERNEL_RUN_COMPLETE_FILE}" "completion report:" "Kernel run-complete emits user-visible completion report"
assert_contains "${KERNEL_RUN_COMPLETE_FILE}" "status: completed" "Kernel run-complete reports completion status"
assert_contains "${KERNEL_RUN_COMPLETE_FILE}" "changed:" "Kernel run-complete reports changed work"
assert_contains "${KERNEL_RUN_COMPLETE_FILE}" "locations:" "Kernel run-complete reports changed locations"
assert_contains "${KERNEL_RUN_COMPLETE_FILE}" "verification:" "Kernel run-complete reports verification"
assert_contains "${KERNEL_RUN_COMPLETE_FILE}" "dirty worktree entries:" "Kernel run-complete reports dirty worktree entries"
assert_contains "${KERNEL_RUN_COMPLETE_FILE}" "dirty-worktree-summary.sh" "Kernel run-complete uses dirty worktree summary helper"

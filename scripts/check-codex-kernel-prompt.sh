...
assert_contains "${CODEX_FILE}" "On \`Mac mini\`, bare \`codex\` inside the Kernel repo should ask \`Kernelを起動しますか? [Y/n]\`" "CODEX documents codex auto-ask"
...
assert_contains "${README_FILE}" "\`Mac mini\` では repo 内で bare \`codex\` を打つと \`Kernelを起動しますか? [Y/n]\` を確認し、\`yes\` なら \`kernel\`、\`no\` なら raw Codex に留まる" "README documents codex auto-ask"

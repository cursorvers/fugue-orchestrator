...
assert_contains "${PROMPT_FILE}" "for non-critical work, continue under local \`/vote\` consensus by default" "prompt requires local vote consensus for non-critical work"
...
assert_contains "${PROMPT_FILE}" "Once a successful local auth, unlock, or trust proof exists in the run, reuse it across lanes" "prompt reuses auth proof across lanes"
assert_contains "${PROMPT_FILE}" "For non-critical work with \`ok_to_execute=true\`, approved local \`/vote\` or Tutti consensus satisfies routine user approval" "prompt allows non-critical consensus approval"
...
assert_contains "${ALIAS_FILE}" "for non-critical work, continue under local \`/vote\` consensus by default" "alias prompt requires local vote consensus for non-critical work"
...
assert_contains "${ALIAS_FILE}" "Once a successful local auth, unlock, or trust proof exists in the run, reuse it across lanes" "alias prompt reuses auth proof across lanes"
assert_contains "${ALIAS_FILE}" "For non-critical work with \`ok_to_execute=true\`, approved local \`/vote\` or Tutti consensus satisfies routine user approval" "alias prompt allows non-critical consensus approval"
...
assert_contains "${CODEX_FILE}" "Only pause for the user when the next step is critical, destructive, requires external credentials" "CODEX limits pauses to hard blockers"
assert_contains "${CODEX_FILE}" "Non-critical Kernel work should run under local \`/vote\` consensus by default" "CODEX requires vote consensus for non-critical work"
...
assert_contains "${CODEX_FILE}" "After one successful local auth, unlock, or trust proof in a run, continue all non-critical work without re-requesting equivalent user auth." "CODEX forbids repeated auth requests"
assert_contains "${CODEX_FILE}" "Non-critical work with approved consensus evidence must not be escalated into another routine" "CODEX allows non-critical consensus approval"
...
assert_contains "${CODEX_FILE}" "For non-critical work, \`/vote\` should record local consensus evidence for the current run before phase completion proceeds." "CODEX requires vote evidence recording"
...
assert_contains "${README_FILE}" "non-critical な Kernel work は既定で local \`/vote\` 合議で進め" "README requires vote consensus for non-critical work"
...
assert_contains "${README_FILE}" "一度通った local auth / unlock / trust 証跡は run 全体の shared evidence として再利用し" "README reuses auth proof across the run"

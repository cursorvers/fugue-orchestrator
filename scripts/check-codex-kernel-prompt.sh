...
assert_contains "${PROMPT_FILE}" "on MBP prefer \`copilot-cli\` when \`gemini-cli\` reviewer lanes are unhealthy" "prompt defines MBP copilot fallback rule"
...
assert_contains "${ALIAS_FILE}" "on MBP prefer \`copilot-cli\` when \`gemini-cli\` reviewer lanes are unhealthy" "alias prompt defines MBP copilot fallback rule"
...
assert_contains "${CODEX_FILE}" "non-interactive startup paths, adapter fallbacks may prefer \`k open\` or \`k new\` before guard launch" "CODEX documents non-interactive k-first fallback"
...
assert_contains "${README_FILE}" "non-interactive な adapter fallback では resumable run を優先して \`k open\` / \`k new\` が guard launch より先に走ることがある" "README documents non-interactive k-first fallback"

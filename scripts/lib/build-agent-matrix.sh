...
codex_main_model="gpt-5.3-codex"
codex_multi_agent_model="gpt-5.3-codex-spark"
claude_opus_model="claude-opus-4-6"
claude_sonnet4_model="claude-3-7-sonnet-latest"
claude_sonnet6_model="claude-3-5-sonnet-latest"
format="json"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --allow-glm-in-subscription)
      ...
      ;;
    --codex-main-model)
      codex_main_model="${2:-gpt-5.3-codex}"
      shift 2
      ;;
    ...
    --claude-sonnet4-model)
      claude_sonnet4_model="${2:-claude-3-7-sonnet-latest}"
      shift 2
      ;;
    --claude-sonnet6-model)
      claude_sonnet6_model="${2:-claude-3-5-sonnet-latest}"
      shift 2
      ;;
    --format)
      format="${2:-json}"
...
Options:
  ...
  --codex-main-model VALUE          default: gpt-5.3-codex
  --codex-multi-agent-model VALUE   default: gpt-5.3-codex-spark
  --claude-opus-model VALUE         default: claude-opus-4-6
  --claude-sonnet4-model VALUE      default: claude-3-7-sonnet-latest
  --claude-sonnet6-model VALUE      default: claude-3-5-sonnet-latest
...
matrix="$(jq -cn \
  ...
  | if ($multi_agent_mode == "enhanced" or $multi_agent_mode == "max") and $use_glm_baseline then
      .include += [
        {name:"glm-refactor-advisor",provider:"glm",api_url:"https://api.z.ai/api/coding/paas/v4/chat/completions",model:"glm-5.0",agent_role:"refactor-advisor"},
        {name:"glm-general-critic",provider:"glm",api_url:"https://api.z.ai/api/coding/paas/v4/chat/completions",model:"glm-5.0",agent_role:"general-critic"}
      ]
    else . end

...
# T1: GHA subscription strict (default enhanced) -> 13 lanes, no GLM baseline.
t1="$("${BUILDER}" \
  --engine subscription \
  --main-provider codex \
...
# T2: GHA harness enhanced paired -> 18 lanes.
t2="$("${BUILDER}" \
  --engine harness \
  --main-provider codex \
  --assist-provider claude \
  ...
  --allow-glm-in-subscription false \
  --format json)"
assert_eq "$(echo "${t2}" | jq -r '.lanes')" "18" "T2 lanes"
assert_eq "$(echo "${t2}" | jq -r '.use_glm_baseline')" "true" "T2 glm baseline"
assert_lane "${t2}" "claude-main-orchestrator" "T2 secondary main lane"
...
# T3: GHA harness max symphony (claude main + codex assist) -> 19 lanes.
t3="$("${BUILDER}" \
  --engine harness \
  ...
  --allow-glm-in-subscription false \
  --format json)"
assert_eq "$(echo "${t3}" | jq -r '.lanes')" "19" "T3 lanes"
assert_eq "$(echo "${t3}" | jq -r '.main_signal_lane')" "claude-main-orchestrator" "T3 main signal lane"
assert_lane "${t3}" "codex-main-orchestrator" "T3 secondary main lane"
...
# T4: Local hybrid subscription + allow GLM -> 16 lanes.
t4="$("${BUILDER}" \
  --engine subscription \
  --main-provider codex \
  --assist-provider claude \
  ...
  --format json)"
assert_eq "$(echo "${t4}" | jq -r '.lanes')" "16" "T4 lanes"
assert_eq "$(echo "${t4}" | jq -r '.use_glm_baseline')" "true" "T4 glm baseline"
assert_lane "${t4}" "claude-main-orchestrator" "T4 secondary main lane"
...
# T5: Local hybrid standard/off -> 9 lanes.
t5="$("${BUILDER}" \
  --engine subscription \
  --main-provider codex \
  --assist-provider claude \
  ...
  --allow-glm-in-subscription true \
  --format json)"
assert_eq "$(echo "${t5}" | jq -r '.lanes')" "9" "T5 lanes"
assert_lane "${t5}" "claude-main-orchestrator" "T5 secondary main lane"
